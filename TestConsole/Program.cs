    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                MessageQueue MSMQ = new MessageQueue(@"FormatName:DIRECT=TCP:192.168.0.81\Private$\can_send_queue");
                MSMQ.ReceiveCompleted += sendMqReceiveCompleted;
                MSMQ.BeginReceive();
                MSMQ.Send(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 0x22 });
                //p.ConnectCAN();
                Console.Read();
            }
            catch (Exception exp)
        ...
        private void InitMQ()
        {
            ...
            sendMQ = new MsmqUtil(string.Format(@"FormatName:DIRECT=TCP:{0}\Private$\{1}", can_send_queue, SEND_NAME));
            rcvMQ = new MsmqUtil(string.Format(@"FormatName:DIRECT=TCP:{0}\Private$\{1}", can_rcv_queue, RCV_NAME));
